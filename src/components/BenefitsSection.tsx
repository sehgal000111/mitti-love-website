...
const benefits: Benefit[] = [
  {
    icon: <Droplet className="h-8 w-8 text-eco-dark" />,
  ...
  {
    icon: <Leaf className="h-8 w-8 text-leaf-dark" />,
...
const BenefitsSection = () => {
  return (
    <section id="benefits" className="page-section bg-earth-light/30">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {benefits.map((benefit, index) => (
            <div 
              key={index} 
              className="bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow flex flex-col items-center text-center hover:scale-[1.02] transition-transform duration-300 leaf-border"
            >
              <div className="mb-4 bg-leaf-light/40 p-4 rounded-full">
                {benefit.icon}
              </div>
      ...
      <div className="organic-separator mt-16"></div>
    </section>
  );
};
